...
- [ ] 1.6 Verify Docker daemon is accessible from Python via `docker` SDK (`docker.from_env()` succeeds)
...
- [ ] 4.6 Run `build_index.py` to build the initial index and verify ≥500 chunks indexed
...
- [ ] 5.6 Write integration test: provide a sample science article and assert valid storyboard JSON is returned
...
- [ ] 6.8 Write integration test with OpenAI TTS: synthesize a short sentence and assert `AudioMeta.duration_seconds` matches `librosa.get_duration`
...
- [ ] 7.5 Write unit test: given a mock scene and duration, assert generated code contains a Manim `Scene` subclass with a `construct()` method
...
- [ ] 8.8 Write integration test: render a simple known-good Manim scene and assert MP4 file is produced with correct resolution
...
- [ ] 9.4 Verify synchronization: the composed video's scene N audio track starts within 50ms of scene N video clip (per spec)
- [ ] 9.5 Write integration test: compose 2 minimal scene clips and assert output MP4 duration equals sum of audio durations
...
- [ ] 13.1 Run full end-to-end pipeline with a real 500-word science article: verify storyboard generated, audio synthesized, video rendered, final MP4 composed
- [ ] 13.2 Test HITL revision loop: submit a revision instruction after initial generation, verify only the targeted scene is regenerated and final video is recomposed
- [ ] 13.3 Test error recovery: inject a deliberately broken Manim scene, verify self-correction loop activates, verify HITL error banner is shown after 5 failed retries
- [ ] 13.4 Test state persistence: kill the pipeline mid-run, restart, verify Streamlit loads the saved state correctly
- [ ] 13.5 Verify audio-video synchronization in composed output: check that scene audio starts within 50ms of corresponding video clip
- [ ] 13.6 Test PDF ingestion: run pipeline with a PDF file input end-to-end
- [ ] 13.7 Test URL ingestion: run pipeline with a science article URL end-to-end
- [ ] 13.8 Verify 720p default render resolution and 1080p flag behavior
